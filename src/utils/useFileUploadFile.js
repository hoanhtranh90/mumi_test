...
const selectFile = (isImageOnly, fileType) =>
...
export default function(additionalData = {}, fileType = DocumentPickerUtil.allFiles()) {
  ...
  const actions = {
    upload: async isImageOnly => {
      let res;
      try {
        res = await selectFile(isImageOnly, fileType);
      } catch (e) {
        return;
      }
      ...
      try {
        ...
        dispatch({ type: 'add', payload: attachment });
      } catch (err) {
