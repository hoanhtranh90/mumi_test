...
import ReactNative, { View, Text, TouchableOpacity, FlatList, ViewPropTypes, Dimensions } from 'react-native';
...
export default class RNSketchCanvas extends React.Component {
    ...
    if (this.state.idx !== this.props.idx) {
      ...
      if (this.scrollResponderRef)
        this.scrollResponderRef.scrollResponderZoomTo({
          x: 0,
          y: 0,
          width: this.props.canvasStyle.width,
          height: this.props.canvasStyle.height,
          animated: true,
        });
    }
  }
  ...
  render() {
    const { onZoomScaleChange, scrollable, strokeWidthChange, touchEnabled } = this.props;
    return (
      <View style={this.props.containerStyle}>
        ...
        <View style={{ zIndex: 1000 }}>
          {this.props.strokeWidthComponent && (
            ...
            >
              {this.props.strokeWidthComponent(strokeWidthChange ? 4 : 0)}
            </TouchableOpacity>
          )}
        ...
        <ScrollView
          ...
          onScroll={event => {
            const nativeEventDrag = {
              x: event.nativeEvent.contentOffset.x,
              y: event.nativeEvent.contentOffset.y,
              zoomScale: event.nativeEvent.zoomScale,
            };
            this.setState({ scaleZoom: nativeEventDrag.zoomScale });
            onZoomScaleChange(nativeEventDrag);
          }}
          style={{
            alignSelf: 'center',
          }}
        >
          <SketchCanvas
            ...
            strokeWidth={strokeWidthChange ? 4 : 0}
