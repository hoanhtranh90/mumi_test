  ...
  componentWillMount() {
    this.panResponder = PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        if (this._path) {
          const x = parseFloat(
              (gestureState.x0 + gestureState.dx / this.props.scaleZoom - this._offset.x).toFixed(2)
            ),
            y = parseFloat(
              (gestureState.y0 + gestureState.dy / this.props.scaleZoom - this._offset.y).toFixed(2)
            );
          UIManager.dispatchViewManagerCommand(
            this._handle,
            UIManager.RNSketchCanvas.Commands.addPoint,
            [parseFloat(x * this._screenScale), parseFloat(y * this._screenScale)]
          );
          this._path.data.push(`${x},${y}`);
...
SketchCanvas.MAIN_BUNDLE =
  Platform.OS === 'ios' ? UIManager.RNSketchCanvas.Constants.MainBundlePath : '';
SketchCanvas.DOCUMENT =
  Platform.OS === 'ios' ? UIManager.RNSketchCanvas.Constants.NSDocumentDirectory : '';
SketchCanvas.LIBRARY =
  Platform.OS === 'ios' ? UIManager.RNSketchCanvas.Constants.NSLibraryDirectory : '';
SketchCanvas.CACHES =
  Platform.OS === 'ios' ? UIManager.RNSketchCanvas.Constants.NSCachesDirectory : '';
