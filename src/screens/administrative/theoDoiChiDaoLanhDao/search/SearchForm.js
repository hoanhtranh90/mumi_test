...
const SearchForm = ({ state, setValue, query }) => {
  const initSearch = () => {
    if (state.meeting) {
      setValue('meeting', null);
    }
    ...
    if (state.progress) {
      _.forEach(PROGRESS, obj => {
        if (obj.value === query.progress) {
          setValue('progress', null);
        }
      });
    }

    if (state.commandsStatus) {
      _.forEach(COMMAND_STATUS, obj => {
        if (obj.value === query.commandsStatus) {
          setValue('commandsStatus', null);
        }
      });
    }

    if (state.startTimeCommand) {
      setValue('startTimeCommand', null);
    }

    if (state.endTimeCommand) {
      setValue('endTimeCommand', null);
    }

    if (state.startTimeDeadline) {
      setValue('startTimeDeadline', null);
    }

    if (state.endTimeDeadline) {
      setValue('endTimeDeadline', null);
    }

    if (state.startTimeFinish) {
      setValue('startTimeFinish', null);
    }

    if (state.endTimeFinish) {
      setValue('endTimeFinish', null);
    }
  };
