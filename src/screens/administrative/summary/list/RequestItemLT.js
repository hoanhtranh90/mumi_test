...
const RequestItemLT = ({ request, onPress }) => {
  const createdTime =request.item.createTime ? format(new Date(request.item.createTime), 'dd/MM/yyyy') : format(new Date(), 'dd/MM/yyyy');
  let color = '';
  let statusColor = '';
  ...
  return (
    <TouchableOpacity onPress={onPress}>
      <View style={styles.wrapper}>
        <Text style={styles.title}>{request.item.meetingTitle}</Text>
        <Text style={styles.desc}>{request.item.meetingPlace}</Text>
...
RequestItemLT.propTypes = {
  request: PropTypes.shape({}),
  onPress: PropTypes.func,
};
RequestItemLT.defaultProps = {
  request: null,
  onPress: null,
};

export default RequestItemLT;
