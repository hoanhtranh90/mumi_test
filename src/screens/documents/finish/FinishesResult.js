...
const forwarded = (doc, state) =>
  !!state.docFinished.includes(doc) ? (
...
const FinishResultModal = ({ docUserView, onClose, isVisible, state, isOnline }) => (
      ...
      renderItem={({ item: document }) => (
        <View style={styles.viewColumn}>
          <View style={styles.viewDocument}>
            <View style={styles.viewColumn}>
              {!!state.docFinish.includes(document) ? (
...
FinishResultModal.defaultProps = {
  isOnline: true,
  docUserView: [],
  onClose() {},
  isVisible: false,
};
export default connect(state => ({ isOnline: selectors.isOnlineSelector(state) }))(
  FinishResultModal
);
