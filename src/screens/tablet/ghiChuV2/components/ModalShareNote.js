...
import * as userDeptRoleViewService from '../../../../service/userDeptRoleViewService';
...
const ModalShareNote = ({ isOpen, close, noteId }) => {
  ...
  const loadData = () => {
    ...
    userDeptRoleViewService.loadUserForShareNote(query).then(users => setUserLoaded(users));
  };
  ...
  const share = () => {
    ...
    if (userDeptRoleIds.length > 0)
      notesService.share(noteId, { userDeptRoleIds }).then(res => {
        Toast.show({
          duration: 3000,
          text: 'Chia sẻ ghi chú thành công',
          type: 'success',
        });
        closeModal();
      });
      ...
      <View style={styles.childContainer}>
        ...
        >
          <View style={styles.header}>
            <Text style={styles.headerTitle}>Chia sẻ ghi chú</Text>
          </View>
          <FlatList
            data={userChoosed}
            keyExtractor={item => item.id}
