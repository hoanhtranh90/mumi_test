...
export const getDetailDieuXe = ({ caseMasterId }) =>
  axios.get(`/dieuxe/detail/${caseMasterId}`, null).then(res => {
    return res.data;
  });
...
export const listFreeCars = query =>
  axios.get('/dieuxe/carfree', { params: formatQuery(query) }).then(res => {
    return res.data;
  });

export const listDriver = query =>
  axios.get('/dieuxe/listDriver', { params: formatQuery(query) }).then(res => {
    return res.data;
  });
