...
export const executeMeeting = createOperation({
  process: async ({ dispatch, payload: { meeting, actionName, actionCode } }) => {
    try {
      ...
      Toast.show({
        text: `${actionName} phòng họp đột xuất thành công`,
        type: 'success',
        duration: 3000,
      });
      ...
      Alert.alert('Thông báo', `${actionName} phòng họp đột xuất lỗi`, [
...
export const listFreeRooms = createOperation({
  process({ payload: { startTime, endTime } }) {
    const hcFlow = FLOW_INFO.PHONG_HOP_DX;
    const params = {
      startTime,
      endTime,
      hcFlow,
...
export const listRoomCalendar = createOperation({
  actions: {
    successAction: actions.listRoomCalendarSuccess,
  },
  process({ payload: { startTime, endTime } }) {
    const hcFlow = FLOW_INFO.PHONG_HOP_DX;
    const params = {
    ...
    return detailService.calendarRoom(params);
  },
});
