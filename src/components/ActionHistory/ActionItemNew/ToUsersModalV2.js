...
const ToUsersModalV2 = ({ isVisible, onClose, toUsers }) => (
          ...
          <CardItem style={styles.item} key={index.toString()}>
            {item.type === 'caNhan' && (
              <UserItem
                ...
                data={{ ...item.handler, ...item.receiverForm }}
              />
            )}
            {item.type === 'donVi' && (
              <Text style={styles.dept}>
                {`${index + 1}. ${item.department.deptName} ${
                  item.receiverForm ? `(${PROCESS_TYPE_TEXTS[item.receiverForm.processType]})` : ''
                }`}
              </Text>
            )}
          </CardItem>
