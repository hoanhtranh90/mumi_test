...
const LabelTabDetailRelatedDocNoAttach = ({ type, count, onPress, isMinusNoAttach }) => {
  const labels = {
    0: {
      label: 'Luồng xử lý',
    ...
    2: {
      label: 'Văn bản liên quan không ban hành kèm',
      ...
      {DeviceInfo.isTablet() && (
        <TouchableOpacity style={styles.field} onPress={onPress}>
          ...
          <Right>
            <Icon
              type="Feather"
              name={isMinusNoAttach ? 'plus' : 'minus'}
              style={{ fontSize: 11 }}
            />
          </Right>
        </TouchableOpacity>
...
LabelTabDetailRelatedDocNoAttach.propTypes = {
  type: PropTypes.number.isRequired,
  count: PropTypes.number,
  onPress: PropTypes.func.isRequired,
  isMinusNoAttach: PropTypes.bool,
};

LabelTabDetailRelatedDocNoAttach.defaultProps = {
  count: 0,
  isMinusNoAttach: true,
};

export default LabelTabDetailRelatedDocNoAttach;
